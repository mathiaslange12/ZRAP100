CLASS lhc_zrap100_r_traveltp_3285 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
...
CLASS lhc_zrap100_r_traveltp_3285 IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    ...
    DATA:
      entity           TYPE STRUCTURE FOR CREATE ZRAP100_R_TravelTP_3285,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap_atrav3285 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap_dtrav3285 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setStatusToOpen.
    READ ENTITIES OF zrap100_R_TravelTP_3285 IN LOCAL MODE
         ENTITY Travel
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_3285 IN LOCAL MODE
           ENTITY Travel
           UPDATE FIELDS ( OverallStatus )
           WITH VALUE #( FOR travel IN travels WHERE ( OverallStatus IS INITIAL )
                         ( %tky          = travel-%tky
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZRAP100_R_TravelTP_3285.
*    DATA reduced_fee    TYPE /dmo/booking_fee.
    ...
    LOOP AT travels ASSIGNING FIELD-SYMBOL(<travel>).
*      reduced_fee = <travel>-BookingFee * ( 1 - 3 / 10 ) .
  ...
  METHOD copyTravel.
    DATA:
       travels       TYPE TABLE FOR CREATE zrap100_r_traveltp_3285\\travel.
